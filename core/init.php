...
if ( defined('BLC_ACTIVE') ){
	trigger_error(
		'Eine andere Version von Link Checker ist bereits aktiv. Bitte deaktiviere es, bevor Du dieses aktivierst.',
		E_USER_ERROR
	);
} else {
...
function blc_resynch( $forced = false ){
	...
	if ( $forced ){
		$blclog->info('...erzwungene Resynchronisation eingeleitet');
		
		//Drop all synchronization records
		$wpdb->query("TRUNCATE {$wpdb->prefix}blc_synch");
	} else {
		$blclog->info('...Resynchronisation eingeleitet');
	}
	...
	$blclog->info('...Container-Datensätze (neu) erstellen');
	blcContainerHelper::resynch($forced);
	
	$blclog->info('...Einstellung resync. flags');
	blc_got_unsynched_items();
	
	//All done.
	$blclog->info('Datenbank-Resynchronisation abgeschlossen.');
}
...
function blc_cleanup_database(){
	...
	$blclog->info('...ungültige Containerdatensätze löschen');
	blcContainerHelper::cleanup_containers();
	
	//Delete invalid instances
	$blclog->info('...Löschen ungültiger Linkinstanzen');
	blc_cleanup_instances();
	
	//Delete orphaned links
	$blclog->info('...verwaiste Links löschen');
	blc_cleanup_links();
}
...
if ( $blc_config_manager->options['installation_complete'] ){
	...
	function blc_print_installation_errors(){
		...
		$messages = array(
			'<strong>' . __('Die Installation von Link Checker ist fehlgeschlagen. Versuche das Plugin zu deaktivieren und dann wieder zu aktivieren.', 'psource-link-checker') . '</strong>',
		);

		if ( is_multisite() && is_plugin_active_for_network(plugin_basename(BLC_PLUGIN_FILE)) ) {
			$messages[] = __('Bitte aktiviere das Plugin auf jeder Seite separat. Die Netzwerkaktivierung wird nicht unterstützt.', 'psource-link-checker');
			$messages[] = '';
		}

		if ( ! $blc_config_manager->db_option_loaded ) {
			$messages[] = sprintf(
				'<strong>Fehler beim Laden der Plugin-Einstellungen über die Option "%s".</strong>',
			...
			if ( $serialized_config === null ) {
				$messages[] = "Option ist in der Tabelle {$wpdb->options} nicht vorhanden.";
			} else {
				$messages[] = "Die Option ist in der Tabelle {$wpdb->options} vorhanden und hat den folgenden Wert:";
				$messages[] = '';
