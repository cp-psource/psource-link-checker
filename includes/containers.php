...
class blcContainerManager extends blcModule {
   ...
   */
	function resynch($forced = false){
		trigger_error('Die Funktion blcContainerManager::resynch() muss in einer Unterklasse überschrieben werden', E_USER_ERROR);
	}
...
class blcContainer {
   ...
   */
	function get_wrapped_object($ensure_consistency = false){
		trigger_error('Die Funktion blcContainer::get_wrapped_object() muss in einer Unterklasse überschrieben werden', E_USER_ERROR);
	}	
   ...
   */
	function update_wrapped_object(){
		trigger_error('Die Funktion blcContainer::update_wrapped_object() muss in einer Unterklasse überschrieben werden', E_USER_ERROR);
	}
   ...
   */
	function delete_wrapped_object(){
		trigger_error('Die Funktion blcContainer::delete_wrapped_object() muss in einer Unterklasse überschrieben werden', E_USER_ERROR);
	}
	...
	function trash_wrapped_object(){
		trigger_error('Die Funktion blcContainer::trash_wrapped_object() muss in einer Unterklasse überschrieben werden', E_USER_ERROR);
	}
...
class blcContainerHelper {
   ...
   */
	...
	static function mark_as_unsynched_where($formats, $container_types){
		...
		$q .= implode(' OR ', $pieces);
		$blclog->log('......Abfrage ausführen: ' . $q);

		$start_time = microtime(true);
		$rez = ($wpdb->query($q) !== false);
		$blclog->log(sprintf('......%d Zeilen betroffen, %.3f Sekunden', $wpdb->rows_affected, microtime(true) - $start_time));
	...
	static function cleanup_containers(){
		...
		$blclog->log(sprintf('...%d Synchronisierungsdatensätze in %.3f Sekunden gelöscht', $wpdb->rows_affected, $elapsed));
   ...
   */
	static function ui_bulk_delete_message($container_type, $n){
		...
		if ( is_null($manager) ){
			return sprintf(__('Containertyp "%s" wird nicht erkannt', 'psource-link-checker'), $container_type);
		} else {
