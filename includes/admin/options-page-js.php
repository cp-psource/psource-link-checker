...
jQuery(function($){
		...
		$.getJSON(
			...
			function (data, textStatus){
				if ( data && ( typeof(data['text']) != 'undefined' ) ){
					...
					$('#wsblc_full_status').html('<?php _e('[ Netzwerkfehler ]', 'psource-link-checker'); ?>');
				}
	...
	toggleButton.on('click', function(){
		...
		box.toggle();
		if( box.is(':visible') ){
			toggleButton.text('<?php _e('Debug-Informationen ausblenden', 'psource-link-checker'); ?>');
		} else {
			toggleButton.text('<?php _e('Debug-Informationen anzeigen', 'psource-link-checker'); ?>');
		}
