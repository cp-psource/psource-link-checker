	...
	class blcTablePrinter {
		...
		function navigation( $table_compact = false, $suffix = '' ) {
			...
			echo '<div class="tablenav">
					...
					<input type="submit" name="doaction' . $suffix . '" id="doaction' . $suffix . '" value="',
						esc_attr( __( 'Anwenden', 'psource-link-checker' ) ) .
			...
			<div class="view-switch">
				<a
					...
					title="<?php echo esc_attr( __( 'Kompakte Ansicht', 'psource-link-checker' ) ); ?>">
				</a>
				<a
					href="<?php echo esc_url( add_query_arg( 'compact', '0', $_SERVER['REQUEST_URI'] ) ) ?>"
					class="view-excerpt <?php if ( ! $table_compact ) echo 'current'; ?>"
					title="<?php echo esc_attr( __( 'Detaillierte Ansicht', 'psource-link-checker' ) ); ?>">
				</a>
			</div>
		...
		*/
		function setup_columns(){
			$this->columns = array(
				'status' => array(
					'heading' => __( 'Status', 'psource-link-checker' ),
					'content' => array( $this, 'column_status' ),
				),

				'new-url' => array(
					'heading' => __( 'URL', 'psource-link-checker' ),
				...
				'used-in' => array(
					'heading' => __( 'Quelle', 'psource-link-checker' ),
					'class' => 'column-title',
					'content' => array( $this, 'column_used_in' ),
				),

				'new-link-text' => array(
					'heading' => __( 'Linktext', 'psource-link-checker' ),
				...
				'redirect-url' => array(
					'heading' => __( 'Umleitungs URL', 'psource-link-checker' ),
		...
		*/
		function prepare_nav_html(){
			...
			$bulk_actions = array(
				'-1' => __( 'Massenaktionen', 'psource-link-checker' ),
				'bulk-edit' => __( 'URL bearbeiten', 'psource-link-checker' ),
				'bulk-recheck' => __( 'Erneut prüfen', 'psource-link-checker' ),
				'bulk-deredirect' => __( 'Weiterleitungen korrigieren', 'psource-link-checker' ),
				'bulk-not-broken' => __( 'Als nicht defekt markieren', 'psource-link-checker' ),
				'bulk-dismiss' => __( 'Verwerfen', 'psource-link-checker' ),
				'bulk-unlink' => __( 'Link aufheben', 'psource-link-checker' ),
			);
			if ( EMPTY_TRASH_DAYS ) {
				$bulk_actions['bulk-trash-sources'] = __( 'Verschiebe Quellen in den Papierkorb', 'psource-link-checker' );
			} else {
				$bulk_actions['bulk-delete-sources'] = __( 'Quellen löschen', 'psource-link-checker' );
			}
			...
			if ( $page_links ) {
				...
				$this->pagination_html .= sprintf(
					'<span class="displaying-num">' . __( 'Zeige %s&#8211;%s von <span class="current-link-count">%s</span>', 'psource-link-checker' ) . '</span>%s',
		...
		*/
		function bulk_edit_form( $visible_columns ) {
			...
			<fieldset>
				<h4><?php _e( 'Massenbearbeitungs-URLs', 'psource-link-checker' ); ?></h4>
				<label>
					<span class="title"><?php _e( 'Finden', 'psource-link-checker' ); ?></span>
					<input type="text" name="search" class="text">
				</label>
				<label>
					<span class="title"><?php _e( 'Ersetzen mit', 'psource-link-checker' ); ?></span>
				...
				<div id="bulk-edit-options">
					...
					<label>
						...
						<?php _e( 'Groß- und Kleinschreibung beachten', 'psource-link-checker' ); ?>
					</label>
					<label>
						<input type="checkbox" name="regex">
						<?php _e( 'Regulärer Ausdruck', 'psource-link-checker' ); ?>
					</label>
				</div>
			</fieldset>

			<p class="submit inline-edit-save">
				<a href="#bulk-edit" class="button-secondary cancel alignleft" title="<?php echo esc_attr( __( 'Abbrechen', 'psource-link-checker' ) ); ?>" accesskey="c"><?php _e( 'Abbrechen', 'psource-link-checker' ); ?></a>
				<input type="submit" name="bulk_edit" class="button-primary alignright" value="<?php
					_e( 'Aktualisieren', 'psource-link-checker' );
				?>" accesskey="s">
			</p>
			</div>
			</td></tr>
		...
		public static function details_row_contents( $link ) {
			...
			<div class="blc-detail-container">
				<div class="blc-detail-block" style="float: left; width: 49%;">
					...
					<li><strong><?php _e( 'Beitrag veröffentlicht am', 'psource-link-checker' ); ?>:</strong>
					<span class='post_date'><?php
						echo date_i18n( get_option( 'date_format' ), strtotime( $link->post_date ) );
					?></span></li>
					<?php } ?>
					<li><strong><?php _e( 'Link zuletzt überprüft', 'psource-link-checker' ); ?>:</strong>
					<span class='check_date'><?php
						$last_check = $link->last_check;
						if ( $last_check < strtotime( '-10 years' ) ) {
							_e( 'noch nie', 'psource-link-checker' );
						} else {
							printf(
					...
					<li><strong><?php _e( 'HTTP Code', 'psource-link-checker' ); ?>:</strong>
					<span class='http_code'><?php
						print $link->http_code;
					?></span></li>

					<li><strong><?php _e( 'Reaktionszeit', 'psource-link-checker' ); ?>:</strong>
					<span class='request_duration'><?php
						printf( __( '%2.3f Sekunden', 'psource-link-checker' ), $link->request_duration );
					?></span></li>

					<li><strong><?php _e( 'Ziel URL', 'psource-link-checker' ); ?>:</strong>
					<span class='final_url'><?php
						print esc_html( $link->final_url );
					?></span></li>

					<li><strong><?php _e( 'Umleitungszähler', 'psource-link-checker' ); ?>:</strong>
					<span class='redirect_count'><?php
						print $link->redirect_count;
					?></span></li>

					<li><strong><?php _e( 'Instanzanzahl', 'psource-link-checker' ); ?>:</strong>
					...
					<?php
						printf(
							_n('Dieser Link ist %d mal fehlgeschlagen.', 'Dieser Link ist %d mal fehlgeschlagen.', $link->check_count, 'psource-link-checker'),
							$link->check_count
						);
						...
						printf(
							__('Dieser Link ist %s unterbrochen.', 'psource-link-checker'),
				...
				<div class="blc-detail-block" style="float: right; width: 50%;">
					<ol style='list-style-type: none;'>
						<li><strong><?php _e('Protokoll', 'psource-link-checker'); ?>:</strong>
		...
		*/
		function column_status( $link, $instances ) {
			printf(
				...
				esc_attr( __( 'Weitere Informationen zu diesem Link anzeigen', 'psource-link-checker' ) )
			);
			...
			if ( $link->last_check != 0 ) {
				$last_check = _x( 'Überprüft', 'Überprüfung vor', 'psource-link-checker' ) . ' ';
			...
			if ( $link->broken ) {
				...
				printf(
					...
					__( 'Defekt für', 'psource-link-checker' ),
					$broken_for
				);
			}
			...
			printf(
				...
				esc_attr( __( 'Weitere Informationen zu diesem Link anzeigen', 'psource-link-checker' ) ),
				_x( 'Details', 'link in the "Status" column', 'psource-link-checker' )
			);
			echo '</div>';
		}
		...
		*/
		function column_new_url( $link ) {
			...
			$actions['edit'] = "<a href='javascript:void(0)' class='blc-edit-button' title='" . esc_attr( __('Bearbeite diesen Link' , 'psource-link-checker') ) . "'>". __( 'URL bearbeiten' , 'psource-link-checker' ) . '</a>';

			$actions['delete'] = "<a class='submitdelete blc-unlink-button' title='" . esc_attr( __( 'Entferne diesen Link aus allen Beiträgen', 'psource-link-checker') ). "' ".
				"href='javascript:void(0);'>" . __( 'Link aufheben', 'psource-link-checker' ) . '</a>';

			if ( $link->broken || $link->warning ) {
				$actions['blc-discard-action'] = sprintf(
					'<a href="#" title="%s" class="blc-discard-button">%s</a>',
					esc_attr( __( 'Entferne diesen Link aus der Liste der defekten Links und markiere ihn als gültig', 'psource-link-checker' ) ),
					__( 'Nicht defekt', 'psource-link-checker' )
				);
			}
			...
			if ( ! $link->dismissed && ( $link->broken || $link->warning || ( $link->redirect_count > 0 ) ) ) {
				$actions['blc-dismiss-action'] = sprintf(
					...
					esc_attr( __( 'Blende diesen Link aus und melde ihn erst wieder, wenn sich sein Status ändert' , 'psource-link-checker' ) ),
					__( 'Ausblenden', 'psource-link-checker' )
				);
			} else if ( $link->dismissed ) {
				$actions['blc-undismiss-action'] = sprintf(
					'<a href="#" title="%s" class="blc-undismiss-button">%s</a>',
					esc_attr( __( 'Diesen Link einblenden', 'psource-link-checker' ) ),
					__( 'Einblenden', 'psource-link-checker' )
				);
			}

			$actions['blc-recheck-action'] = sprintf(
				'<a href="#" class="blc-recheck-button">%s</a>',
				__( 'Erneut prüfen', 'psource-link-checker' )
			);
			...
			if ( $link->redirect_count > 0 && ! empty( $link->final_url ) && ( $link->url != $link->final_url ) ) {
				...
				$actions['blc-deredirect-action'] = sprintf(
					...
					__( 'Ersetze diese Weiterleitung durch einen direkten Link', 'psource-link-checker' ),
					_x( 'Umleitung korrigieren', 'link action; replace one redirect with a direct link', 'psource-link-checker' )
				);
			}
			...
			<div class="blc-url-editor-buttons">
				<input type="button" class="button-secondary cancel alignleft blc-cancel-button" value="<?php echo esc_attr( __( 'Abbrechen', 'psource-link-checker' ) ); ?>" />
				<input type="button" class="button-primary save alignright blc-update-url-button" value="<?php echo esc_attr( __( 'URL aktualisieren', 'psource-link-checker' ) ); ?>" />
		...
		*/
		function column_used_in( $link, $instances ) {
			...
			if ( ! empty( $instances ) ) {
				...
				_e( '[Ein verwaister Link! Dies ist ein Fehler.]', 'psource-link-checker' );
			}
		}
		...
		*/
		...
		protected function inline_editor( $visible_columns ) {
				...
				<tr id="blc-inline-edit-row" class="blc-inline-editor">
					<td class="blc-colspan-change" colspan="<?php echo count( $visible_columns ); ?>">
						<div class="blc-inline-editor-content">
							<h4><?php echo _x( 'Link bearbeiten', 'inline editor title', 'psource-link-checker' ); ?></h4>

							<label>
								<span class="title"><?php echo _x( 'Text', 'inline link editor', 'psource-link-checker' ); ?></span>
								<span class="blc-input-text-wrap"><input type="text" name="link_text" value="" class="blc-link-text-field" /></span>
							</label>

							<label>
								<span class="title"><?php echo _x( 'URL', 'inline link editor', 'psource-link-checker' ); ?></span>
							...
							<div class="blc-url-replacement-suggestions" style="display: none;">
								<h4><?php echo _x( 'Vorschläge', 'inline link editor', 'psource-link-checker' ); ?></h4>
							...
							<div class="submit blc-inline-editor-buttons">
								<input type="button" class="button-secondary cancel alignleft blc-cancel-button" value="<?php echo esc_attr( __( 'Abbrechen', 'psource-link-checker' ) ); ?>" />
								<input type="button" class="button-primary save alignright blc-update-link-button" value="<?php echo esc_attr( __( 'Aktualisieren', 'psource-link-checker' ) ); ?>" />
			...
			<ul id="blc-suggestion-template" style="display: none;">
				<li>
					<input type="button" class="button-secondary blc-use-url-button" value="<?php echo esc_attr( __( 'Verwende diese URL', 'psource-link-checker' ) ); ?>" />
