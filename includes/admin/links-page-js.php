...
function reloadDetailsRow(link_id){
	...
	details_row.find('td').html('<center><?php echo esc_js(__('Wird geladen...' , 'psource-link-checker')); ?></center>').load(
...
jQuery(function($){
        ...
        $.post(
			...
			function (data, textStatus){
				if (data == 'OK'){
					...
					me.html('<?php echo esc_js(__('Nicht defekt' , 'psource-link-checker'));  ?>');
					alert(data);
				}
			}
		);
	...
	function showLinkEditor(link_id) {
		...
		var linkText = master.data('link-text'),
			...
			noneText = '<?php echo esc_js(_x('(Keine)', 'link text', 'psource-link-checker')); ?>',
			multipleLinksText = '<?php echo esc_js(_x('(Mehrere Links)', 'link text', 'psource-link-checker')); ?>';
	...
	function findReplacementSuggestions(url, suggestionList) {
		var searchingText     = '<?php echo esc_js(_x('Suchen...', 'link suggestions', 'psource-link-checker')) ?>';
		var noSuggestionsText = '<?php echo esc_js(_x('Keine Vorschläge verfügbar.', 'link suggestions', 'psource-link-checker')) ?>';
		var iaSuggestionName  = '<?php echo esc_js(_x('Archivierte Seite von %s (über die Wayback-Maschine)', 'link suggestions', 'psource-link-checker')); ?>';
		...
		if (response && (typeof(response['error']) != 'undefined')){
			...
			if ( response.cnt_okay > 0 ){
				var fragment = sprintf(
					'<?php echo esc_js(__('%d Instanzen des Links wurden erfolgreich geändert.', 'psource-link-checker')); ?>',
					response.cnt_okay
				);
				msg = msg + fragment + '\n';
				if ( response.cnt_error > 0 ){
					fragment = sprintf(
						'<?php echo esc_js(__("%d Instanzen konnten jedoch nicht bearbeitet werden und verweisen weiterhin auf die alte URL.", 'psource-link-checker')); ?>',
						response.cnt_error
					);
					msg = msg + fragment + "\n";
				}
			} else {
				msg = msg + '<?php echo esc_js(__('Der Link konnte nicht geändert werden.', 'psource-link-checker')); ?>\n';
			}

			msg = msg + '\n<?php echo esc_js(__("Folgende Fehler sind aufgetreten:", 'psource-link-checker')); ?>\n* ';
	...
	$('.blc-update-link-button').on('click', function() {
		...
		if ($.trim(newUrl) == '') {
			alert('<?php echo esc_js(__('Fehler: Die Link-URL darf nicht leer sein.', 'psource-link-checker')); ?>');
			urlField.focus();
			return;
		}
			...
			function (data, textStatus){
				...
				if ( data && (typeof(data['error']) != 'undefined') ){
					...
					if ( data.errors.length == 0 ){
						...
						if ( data.cnt_okay > 0 ){
							msg = msg + sprintf(
								'<?php echo esc_js(__("%d Instanzen der Verknüpfung wurden erfolgreich getrennt.", 'psource-link-checker')); ?>\n', 
								data.cnt_okay
							);
							
							if ( data.cnt_error > 0 ){
								msg = msg + sprintf(
									'<?php echo esc_js(__("%d Instanzen konnten jedoch nicht entfernt werden.", 'psource-link-checker')); ?>\n',
									data.cnt_error
								);
							}
						} else {
							msg = msg + '<?php echo esc_js(__("Das Plugin konnte den Link nicht entfernen.", 'psource-link-checker')); ?>\n';
						}
				...
				$(me).html('<?php echo esc_js(__('Verknüpfung aufheben' , 'psource-link-checker')); ?>'); 
			}
		);
    });
	...
	$('#blc-delete-filter').on('click', function(){
		...
		echo esc_js(
			html_entity_decode(
				__('Du bist dabei, den aktuellen Filter zu löschen. "Abbrechen" zum Stoppen "OK" zum Löschen', 'psource-link-checker'),
				ENT_QUOTES,
    ...
    $('#blc-bulk-action-form').submit(function(){
    	...
    	if ( action == 'bulk-delete-sources' ){
    		...
    		message = '<?php
				echo esc_js(  
					html_entity_decode(
						__('Möchtest Du wirklich alle Beiträge, Lesezeichen oder andere Elemente löschen, die einen der ausgewählten Links enthalten? Diese Aktion kann nicht rückgängig gemacht werden. "Abbrechen" zum Stoppen "OK" zum Löschen', 'psource-link-checker'),
						ENT_QUOTES,
						get_bloginfo('charset')
					)
				); 
			...
			message = '<?php
				echo esc_js(  
					html_entity_decode(
						__('Möchtest Du die ausgewählten Links wirklich entfernen? Diese Aktion kann nicht rückgängig gemacht werden. "Abbrechen" zum Stoppen "OK" zum Löschen', 'psource-link-checker'),
						ENT_QUOTES,
						get_bloginfo('charset')
					)
				); 
	...
	$('#bulk-edit input[type="submit"]').on('click', function(e){
		...
		if ($('tbody th.check-column input:checked').length == 0){
			alert('<?php echo esc_js(__('Wähle einen oder mehrere Links zum Bearbeiten aus.', 'psource-link-checker')); ?>');
			e.preventDefault();
		}
	});
