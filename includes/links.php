...
class blcLink {
	...
	var $http_status_codes = array(
        ...
        416=>'Angeforderter Bereich nicht zufriedenstellend',
	...
	function __construct($arg = null){
		...
		if (is_numeric($arg)){
			...
			if ( is_array($arr) ){ //Loaded successfully
				...
				$blclog->warn(__CLASS__ .':' . __FUNCTION__ . ' Link nicht gefunden.', $arg);
			}
...
//				$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Link not found.');
   ...
   */
	function check( $save_results = true ){
        ...
        */
        if ( $this->being_checked ) {
        	...
        	$error_notice = "[" . __("Das Plugin-Skript wurde beendet, als versucht wurde, den Link zu überprüfen.", 'psource-link-checker') . "]";
		...
		if ( $save_results ) {
	        ...
	        //Aktualisiert den DB-Datensatz, bevor die Prüfung tatsächlich durchgeführt wird.
	        //Nützlich, wenn beim Überprüfen dieser bestimmten URL ein furchtbarer Fehler auftritt
			//(z.B. beendet der Server möglicherweise das Skript, um das Ausführungszeitlimit zu überschreiten.).
	        //Hinweis: Möglicherweise nicht erforderlich.
	        $this->save();
        }
		...
		if ( is_null($checker) ){
			...
			$this->broken = false;
			$this->being_checked = false;
			$this->log = __("Das Plugin kann diese Art von Link nicht überprüfen.", 'psource-link-checker');
	...
	private function decide_warning_state($check_results) {
		...
		if ( $check_results['timeout'] ) {
			...
			$warning_reason = 'Zeitüberschreitungen werden manchmal durch hohe Serverlast oder andere vorübergehende Probleme verursacht.';
		}

		$error_code = isset($check_results['error_code']) ? $check_results['error_code'] : '';
		if ( $error_code === 'connection_failed' ) {
			$maybe_temporary_error = true;
			$warning_reason = 'Verbindungsfehler werden manchmal durch hohe Serverlast oder andere vorübergehende Probleme verursacht.';
		}
		...
		if ( in_array($http_code, $temporary_http_errors) ) {
			...
			if ( in_array($http_code, array(502, 503, 504, 509)) ) {
				$warning_reason = sprintf(
					'HTTP-Fehler %d bedeutet normalerweise, dass die Webseite aufgrund einer hohen Serverlast oder eines Konfigurationsproblems nicht verfügbar ist. '
					. 'Dieser Fehler ist oft nur vorübergehend und verschwindet nach einiger Zeit.',
					$http_code
				);
			} else {
				$warning_reason = 'Dieser HTTP-Fehler ist häufig vorübergehend.';
			}
		}
		...
		if ( $is_internal_link && ($http_code == 403) ) {
			...
			$warning_reason = 'Dies könnte falsch positiv sein. Stelle sicher, dass der Link nicht durch ein Passwort geschützt ist, '
				. 'und dass Dein Server nicht zum Blockieren automatisierter Anforderungen eingerichtet ist.';
		}
		...
		if ( $is_internal_link && in_array($error_code, array('connection_failed', 'couldnt_resolve_host')) ) {
			$suspected_false_positive = true;
			$warning_reason = 'Dies ist wahrscheinlich falsch positiv. ';
			if ( $error_code === 'connection_failed' ) {
				$warning_reason .= 'Das Plugin konnte keine Verbindung zu Deiner Webseite herstellen. Das bedeutet normalerweise, dass Dein '
					. 'Hosting-Anbieter die Loopback-Verbindungen deaktiviert hat.';
			} elseif ( $error_code === 'couldnt_resolve_host' ) {
				$warning_reason .= 'Das Plugin konnte keine Verbindung zu Deiner Webseite herstellen, da die DNS-Auflösung fehlgeschlagen ist. '
					. 'Dies kann bedeuten, dass DNS auf Deinem Server falsch konfiguriert ist.';
			}
		}
   ...
   */
	private function status_changed($broken, $new_result_hash = ''){
		...
		if ( $this->result_hash != $new_result_hash ) {
			if ( $this->dismissed ) {
				$this->log .= sprintf(
					"Wiederherstellen eines entlassenen Links. \nAlter Status: \n%s\nNeuer Status: \n%s\n",
					$this->result_hash,
					$new_result_hash
				);
			}
		...
		if ( $this->broken || $this->warning ) {
			$this->log .= "\n" . __("Link ist unterbrochen.", 'psource-link-checker');
		} else {
			$this->log .= "\n" . __("Link ist gültig.", 'psource-link-checker');
		}
	}
   ...
   */
	function save(){
		...
		if ( $this->is_new ){
			...
			$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Neuen Link hinzufügen. SQL-Abfrage:' . "\n", $q);

			$rez = $wpdb->query($q) !== false;

			if ($rez){
				$this->link_id = $wpdb->insert_id;
				$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Datenbankdatensatz erstellt. ID = ' . $this->link_id);
				//FB::info($this->link_id, "Link added");
				//If the link was successfully saved then it's no longer "new"
				$this->is_new = false;
			} else {
				$blclog->error(__CLASS__ .':' . __FUNCTION__ . ' Fehler beim Hinzufügen des Links', $this->url);
				//FB::error($wpdb->last_error, "Fehler beim Hinzufügen des Links {$this->url}");
			}
			...
			$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Link aktualisieren. SQL-Abfrage:'. "\n", $q);

			$rez = $wpdb->query($q) !== false;
			if ( $rez ){
				//FB::log($this->link_id, "Link updated");
				$blclog->debug(__CLASS__ .':' . __FUNCTION__ . ' Link aktualisiert.');
			} else {
				$blclog->error(__CLASS__ .':' . __FUNCTION__ . ' Fehler beim Aktualisieren des Links', $this->url);
   ...
   */
	function edit($new_url, $new_text = null){
		if ( !$this->valid() ){
			return new WP_Error(
				'link_invalid',
				__("Link ist ungültig", 'psource-link-checker')
			);
		}
		...
		if (empty($instances)) {
			return array(
				...
				'errors' => array(
					new WP_Error(
						'no_instances_found',
						__('Dieser Link kann nicht bearbeitet werden, da er nirgendwo auf dieser Webseite verwendet wird.', 'psource-link-checker')
					)
				)
			);
		};
			...
			return array(
				...
				'errors' => array(
					new WP_Error(
						...
						__('Fehler beim Erstellen eines DB-Eintrags für die neue URL.', 'psource-link-checker')
					)
				)
			);
		}
   ...
   */
	function deredirect(){
		...
		if ( ($this->redirect_count <= 0) || empty($this->final_url) ){
			return new WP_Error(
				'not_redirect',
				__("Dieser Link ist keine Weiterleitung", 'psource-link-checker')
			);
		}
			...
			if ( $rez ){
				...
				return array(
					...
					'errors' => array(
						new WP_Error(
							"deletion_failed",
							__("Der Datenbankdatensatz des Links konnte nicht gelöscht werden", 'psource-link-checker')
						)
					),
				);
			}
   ...
   */
	...
	function analyse_status(){
		...
		$text = _x('Unbekannt', 'link status', 'psource-link-checker');
		...
		if ( isset($this->status_text) && !empty($this->status_text) && !empty($this->status_code) ){
			...
			if ( $this->broken || $this->warning ){
				...
				$text = __('Unbekannter Fehler', 'psource-link-checker');

				if ( $this->timeout ){

					$text = __('Zeitüberschreitung', 'psource-link-checker');
					$code = BLC_LINK_STATUS_WARNING;
				...
				if ( !$this->last_check ) {
					$text = __('Nicht geprüft', 'psource-link-checker');
					$code = BLC_LINK_STATUS_UNKNOWN;
				} elseif ( $this->false_positive ) {
					$text = __('Falsch positiv', 'psource-link-checker');
					$code = BLC_LINK_STATUS_UNKNOWN;
				} else {
					$text = _x('OK', 'link status', 'psource-link-checker');
...
function blc_cleanup_links( $link_id = null ){
	...
	$blclog->log(sprintf('...%d Links in %.3f Sekunden gelöscht', $wpdb->rows_affected, $elapsed));
