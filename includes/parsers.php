...
class blcParser extends blcModule {
	...
	function resynch_relevant_containers($only_return = false){
		global $blclog;
		$blclog->log(sprintf('......Parser "%s" markiert relevante Elemente als nicht synchronisiert', $this->module_id));
   ...
   */
	function edit($content, $new_url, $old_url, $old_raw_url){
		return new WP_Error(
			'not_implemented',
			sprintf(__('......Parser "%s" markiert relevante Elemente als nicht synchronisiert', 'psource-link-checker'), $this->parser_type)
		);
	}
   ...
   */
	function unlink($content, $url, $raw_url){
		return new WP_Error(
			'not_implemented',
			sprintf(__('Das Aufheben der Verknüpfung ist im Parser "%s" nicht implementiert', 'psource-link-checker'), $this->parser_type)
		);
	}
   ...
   *
   * ClassicPress 3.4 has WP_Http::make_absolute_url() which is well-tested but not as comprehensive
