   ...
   */
	...
	function edit($new_url, $old_url = '', $new_text = null){
		...
		if ( is_null($container) ){
			return new WP_Error(
				'container_not_found',
				sprintf(__("Container %s[%d] nicht gefunden", 'psource-link-checker'), $this->container_type, $this->container_id)
			);
		}
		...
		if ( is_null($parser) ){
			return new WP_Error(
				'parser_not_found', 
				sprintf(__('Parser "%s" nicht gefunden.', 'psource-link-checker'), $this->parser_type)
			);
		}
...
function blc_cleanup_instances(){
	...
	$blclog->log(sprintf('...%d Instanzen in %.3f Sekunden gelöscht', $wpdb->rows_affected, $elapsed));
	...
	$rez2 = $wpdb->query($q);
	$elapsed = microtime(true) - $start;
	$blclog->log(sprintf('...%d weitere Instanzen in %.3f Sekunden gelöscht', $wpdb->rows_affected, $elapsed));
