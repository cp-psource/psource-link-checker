...
class blcModuleManager {
			...
			if ( $markup || $translate ){
				...
				foreach($this->_category_cache as $category_id => $modules){
					...
					foreach($modules as $module_id => $module_data){
						if ( $translate ){
							$module_data['Name'] = _x($module_data['Name'], 'module name', 'psource-link-checker');
						}
   ...
   */
	...
	function plugin_activated(){
		...
		//Ensure all active modules have the latest headers
		$blclog->log('...Modul-Cache aktualisieren');
		$start = microtime(true);
		$this->refresh_active_module_cache();
		$blclog->info(sprintf('...Die Cache-Aktualisierung dauerte %.3f Sekunden', microtime(true) - $start));
		
		//Notify them that we've been activated
		$blclog->log('... Loading modules');
		$start = microtime(true);
		$this->load_modules();
		$blclog->info(sprintf('...%d Module in %.3f Sekunden geladen', count($this->loaded), microtime(true) - $start));

		$active = $this->get_active_modules();
		foreach($active as $module_id => $module_data){
			$blclog->log( sprintf('...Benachrichtigungsmodul "%s"', $module_id) );
			$module = $this->get_module($module_id);
			if ( $module ){
				$module->plugin_activated();
			} else {
				$blclog->warn(sprintf('...Modul "%s" konnte nicht geladen werden!', $module_id));
			}
		}
	}
	...
	function validate_module($module_data){
		if ( empty($module_data['ModuleID']) ){
			return new WP_Error('invalid_cached_header', 'Der zwischengespeicherte Modulheader ist ungültig');
		}
		
		if ( empty($module_data['virtual']) ){
			//Normal modules must have a valid filename
			if ( empty($module_data['file']) ){
				return new WP_Error('module_not_found', 'Ungültige Moduldatei');
			}
			
			$filename = $this->module_dir . '/' . $module_data['file'];
			if ( !file_exists($filename) ){
				return new WP_Error('module_not_found', 'Moduldatei nicht gefunden');
			}
			...
			if ( !array_key_exists($module_data['ModuleID'], $installed) ){
				return new WP_Error('invalid_module_header', 'Ungültiger Modulheader');
			}
		} else {
			//Virtual modules need to be currently registered
			if ( !array_key_exists($module_data['ModuleID'], $this->_virtual_modules) ){
				return new WP_Error('module_not_registered', 'Das virtuelle Modul ist nicht registriert');
			}
		}
	...
	function normalize_module_header($module_header, $module_id, $module_filename = ''){
		...
		$defaults = array(
			...
			'TextDomain' => 'psource-link-checker', //For translating module headers
		);
