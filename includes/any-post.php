   ...
   */
	...
	function resynch($container_type = '', $forced = false){
		...
		if ( $this->resynch_already_done ){
			$blclog->log(sprintf('......Überspringen der erneuten Synchronisierung von "%s", da alle Beitragstypen bereits synchronisiert wurden.', $container_type));
			return;
		}
		
		if ( empty($this->enabled_post_types) ){
			$blclog->warn(sprintf('......Überspringen der erneuten Synchronisierung von "%s", da keine Beitragstypen aktiviert sind.', $container_type));
			return;
		}
		...
		if ( $forced ){
			...
			$blclog->log('......Erstellen von Synchronisierungsdatensätzen für diese Beitragstypen: '.implode(', ', $escaped_post_types) . ' die einen dieser Status haben: ' . implode(', ', $escaped_post_statuses));
			...
			$q = sprintf(
	 		...
	 		$wpdb->query( $q );
	 		$blclog->log(sprintf('......%d Zeilen in %.3f Sekunden eingefügt', $wpdb->rows_affected, microtime(true) - $start));
 		} else {
 			//Delete synch records corresponding to posts that no longer exist.
 			$blclog->log('......Synchronisierungsdatensätze für entfernte Beiträge löschen');
			$start = microtime(true);
			$q = "DELETE synch.*
				  FROM 
			...
			$blclog->debug($q);
			$blclog->log(sprintf('......%d Zeilen in %.3f Sekunden gelöscht', $wpdb->rows_affected, $elapsed));

			//Delete records where the post status is not one of the enabled statuses.
			$blclog->log('......Löschen von Synchronisierungsdatensätzen für Beiträge, deren Status nicht zulässig ist');
			$start = microtime(true);
			$q = "DELETE synch.*
				  FROM
			...
			$blclog->log('......Geänderte Beiträge als nicht synchronisiert markieren');
			$start = microtime(true);
			$q = "UPDATE
					{$wpdb->prefix}blc_synch AS synch
			...
			$elapsed = microtime(true) - $start;
			$blclog->debug($q);
			$blclog->log(sprintf('......%d Zeilen in %.3f Sekunden aktualisiert', $wpdb->rows_affected, $elapsed));
			
			//Create synch. records for posts that don't have them.
			$blclog->log('......Synchronisierungsdatensätze für neue Beiträge erstellen');
			$start = microtime(true);
			$q = "INSERT INTO {$wpdb->prefix}blc_synch(container_id, container_type, synched)
				  SELECT posts.id, posts.post_type, 0
				  FROM 
			...
			$elapsed = microtime(true) - $start;
			$blclog->debug($q);
			$blclog->log(sprintf('......%d Zeilen in %.3f Sekunden eingefügt', $wpdb->rows_affected, $elapsed));
		}
			...
			if ( current_user_can( $post_type_object->cap->delete_post, $this->container_id ) ){
				if ( $this->can_be_trashed() ) { 
					$actions['trash'] = sprintf(
						...
						esc_attr(__('Verschiebe in den Papierkorb')),
					...
					$actions['delete'] = sprintf(
						...
						esc_attr(__('Lösche diesen Eintrag dauerhaft')),
		...
		if ( in_array($post->post_status, array('pending', 'draft')) ) {
			if ( current_user_can($post_type_object->cap->edit_post, $this->container_id) ){
				$actions['view'] = sprintf(
					...
					esc_attr(sprintf(__('Vorschau &#8220;%s&#8221;'), $title)),
					__('Vorschau')
				);
			}
			...
			$actions['view'] = sprintf(
				...
				esc_attr(sprintf(__('&#8220;%s&#8221; ansehen'), $title)),
				__('Ansehen')
			);
		}
   ...
   */
	function ui_get_source($container_field = '', $context = 'display'){
		...
		$source = sprintf(
			$source,
			$this->get_edit_url(),
			esc_attr(__('Bearbeite diesen Eintrag')),
   ...
   */
	function update_wrapped_object(){
		if ( is_null($this->wrapped_object) ){
			return new WP_Error(
				'no_wrapped_object',
				__('Nichts zu aktualisieren', 'psource-link-checker')
			);
		}
		...
		if ( is_wp_error($post_id) ) {
			...
			return new WP_Error(
				'update_failed',
				sprintf(__('Das Aktualisieren von %d ist fehlgeschlagen', 'psource-link-checker'), $this->container_id)
			);
		} else {
			return true;
		}
		...
		if ( EMPTY_TRASH_DAYS ){
			...
			if ( wp_delete_post($this->container_id, true) ){
				...
				return new WP_Error(
					'delete_failed',
					sprintf(
						__('Fehler beim Löschen des Beitrags "%s" (%d)', 'psource-link-checker'),
						get_the_title($this->container_id),
						$this->container_id
					)
				);
	...
	function trash_wrapped_object(){
		if ( !EMPTY_TRASH_DAYS ){
			return new WP_Error(
				'trash_disabled',
				sprintf(
					__('Der Beitrag "%s" (%d) kann nicht in den Papierkorb verschoben werden, da die Papierkorbfunktion deaktiviert ist', 'psource-link-checker'),
					get_the_title($this->container_id),
					$this->container_id
				)
			);
		...
		if ( wp_trash_post($this->container_id) ){
			...
			return new WP_Error(
				'trash_failed',
				sprintf(
					__('Fehler beim Verschieben des Beitrags "%s" (%d) in den Papierkorb', 'psource-link-checker'),
					get_the_title($this->container_id),
					$this->container_id
				)
			);
...
class blcAnyPostContainerManager extends blcContainerManager {
	...
	function ui_bulk_delete_message($n){
		...
		if ( function_exists('wp_trash_post') && EMPTY_TRASH_DAYS ){
			...
			if ( $this->container_type == 'post' || is_null($post_type_object) ){
				$delete_msg = _n("%d Beitrag gelöscht.", "%d Beiträge gelöscht.", $n, 'psource-link-checker');
			} elseif ( $this->container_type == 'page' ){
				$delete_msg = _n("%d Seite gelöscht.", "%d Seiten gelöscht.", $n, 'psource-link-checker');
			} else {
				$delete_msg = _n('%d "%s" gelöscht.', '%d "%s" gelöscht.', $n, 'psource-link-checker');
   ...
   */
	function ui_bulk_trash_message($n){
		...
		if ( $this->container_type == 'post' || is_null($post_type_object) ){
			$delete_msg = _n("%d Beitrag wurde in den Papierkorb verschoben.", "%d Beiträge wurden in den Papierkorb verschoben.", $n, 'psource-link-checker');
		} elseif ( $this->container_type == 'page' ){
			$delete_msg = _n("%d Seite in den Papierkorb verschoben.", "%d Seiten wurden in den Papierkorb verschoben.", $n, 'psource-link-checker');
		} else {
			$delete_msg = _n('%d "%s" wurde in den Papierkorb verschoben.', '%d "%s" wurden in den Papierkorb verschoben.', $n, 'psource-link-checker');
			$type_name = ($n == 1 ? $post_type_object->labels->singular_name : $post_type_object->labels->name); 
		}
		return sprintf($delete_msg, $n, $type_name);
	}
