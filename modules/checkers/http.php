...
class blcCurlHttp extends blcHttpCheckerBase {
	...
	function check($url, $use_get = false){
        ...
        if ( $result['http_code'] == 0 ){
        	...
        	switch( $error_code ) {
        		case 6: //CURLE_COULDNT_RESOLVE_HOST
		        	...
		        	$result['status_text'] = __('Server Not Found', 'psource-link-checker');
	        	...
	        	case 7: //CURLE_COULDNT_CONNECT
	        		...
	        		if ( $result['request_duration'] >= 0.9*$conf->options['timeout'] ){
	        			...
	        			$result['status_code'] = BLC_LINK_STATUS_WARNING;
	        			$result['status_text'] = __('Connection Failed', 'psource-link-checker');
        ...
        $log .= '=== ';
        if ( $result['http_code'] ){
			$log .= sprintf( __('HTTP code : %d', 'psource-link-checker'), $result['http_code']);
		} else {
			$log .= __('(No response)', 'psource-link-checker');
		}
		$log .= " ===\n\n";
        ...
        if ( !empty($result['broken']) && !empty($result['timeout']) ) {
			$log .= "\n(" . __("Most likely the connection timed out or the domain doesn't exist.", 'psource-link-checker') . ')';
		}
...
class blcSnoopyHttp extends blcHttpCheckerBase {
	...
	function check($url){
        ...
        if ($snoopy->error)
            $log .= $snoopy->error."\n";
        if ($snoopy->timed_out) {
            $log .= __("Request timed out.", 'psource-link-checker') . "\n";
            $result['timeout'] = true;
        }
		...
		$log .= "<em>(" . __('Using Snoopy', 'psource-link-checker') . ")</em>";
		$result['log'] = $log;
