...
class blcAcfMeta extends blcContainer {
    ...
    function update_field($field, $new_value, $old_value = '') {
        ...
        if ($rez) {
            ...
            return new WP_Error('metadata_update_failed', sprintf(__("Failed to update the meta field '%s' on %s [%d]", 'psource-link-checker'), $field, $this->meta_type, $this->container_id));
        }
    }
    ...
    function unlink($field_name, $parser, $url, $raw_url = '') {
        ...
        if ($rez) {
            ...
            return new WP_Error('metadata_delete_failed', sprintf(__("Failed to delete the meta field '%s' on %s [%d]", 'psource-link-checker'), $field_name, $this->meta_type, $this->container_id));
        }
    }
    ...
    function ui_get_action_links($container_field) {
        ...
        $actions['view'] = '<span class="view"><a href="' . esc_url(get_permalink($this->container_id)) . '" title="' . esc_attr(sprintf(__('View "%s"', 'psource-link-checker'), get_the_title($this->container_id))) . '" rel="permalink">' . __('View') . '</a>';
    ...
    function delete_wrapped_object() {
        ...
        if (EMPTY_TRASH_DAYS) {
            ...
            if (wp_delete_post($this->container_id)) {
                ...
                return new WP_Error('delete_failed', sprintf(__('Failed to delete post "%s" (%d)', 'psource-link-checker'), get_the_title($this->container_id), $this->container_id));
            }
        }
    }
    ...
    function trash_wrapped_object() {
        ...
        if (!EMPTY_TRASH_DAYS) {
            return new WP_Error('trash_disabled', sprintf(__('Can\'t move post "%s" (%d) to the trash because the trash feature is disabled', 'psource-link-checker'), get_the_title($this->container_id), $this->container_id));
        }
        ...
        if (wp_trash_post($this->container_id)) {
            ...
            return new WP_Error('trash_failed', sprintf(__('Failed to move post "%s" (%d) to the trash', 'psource-link-checker'), get_the_title($this->container_id), $this->container_id));
        }
    }
