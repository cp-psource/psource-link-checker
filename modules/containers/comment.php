...
class blcComment extends blcContainer{
   ...
   */
	function update_wrapped_object(){
		...
		if ( wp_update_comment($data) ){
			...
			return new WP_Error(
				'update_failed',
				sprintf(__('Updating comment %d failed', 'psource-link-checker'), $this->container_id)
			);
		}
	}
		...
		if ( EMPTY_TRASH_DAYS ){
			...
			if ( wp_delete_comment($this->container_id, true) ){
				...
				return new WP_Error(
					'delete_failed',
					sprintf(
						__('Failed to delete comment %d', 'psource-link-checker'),
						$this->container_id
					)
				);
			}
	...
	function trash_wrapped_object(){
		if ( wp_trash_comment($this->container_id) ){
			...
			return new WP_Error(
				'trash_failed',
				sprintf(
					__('Can\'t move comment %d to the trash', 'psource-link-checker'),
					$this->container_id
				)
			);
		}
   ...
   */
	...
	function ui_get_action_links($container_field){
		...
		$actions['view'] = '<span class="view"><a href="' . get_comment_link($this->container_id) . '" title="' . esc_attr(__('View comment', 'psource-link-checker')) . '" rel="permalink">' . __('View') . '</a>';
