...
class blcRapidShareChecker extends blcChecker {
	...
	function check($url){
		$result = array(
			...
			'timeout' => false,
			'broken' => false,
			'log' => sprintf("<em>(%s)</em>\n\n", __('Using RapidShare API', 'psource-link-checker')),
			'result_hash' => '',
			'status_code' => '',
			'status_text' => '',
		);
				...
				if ( isset($data[4]) ){
					...
					switch( $file_status ){
						case 0:
							...
							$result['status_text'] = __('Not Found', 'psource-link-checker');
							break;
						...
						case 3:
							...
							$result['status_text'] = __('RS Server Down', 'psource-link-checker');
							break;
						...
						case 4:
							...
							$result['broken'] = true;
							$result['status_code'] = BLC_LINK_STATUS_ERROR;
							$result['status_text'] = __('File Blocked', 'psource-link-checker');
							break;
						...
						case 5:
							...
							$result['broken'] = true;
							$result['status_code'] = BLC_LINK_STATUS_WARNING;
							$result['status_text'] = __('File Locked', 'psource-link-checker');
							break;
					...
					$result['log'] .= sprintf(
						__('RapidShare : %s', 'psource-link-checker'),
						$file_status_text
					);
												
				} else {
					$result['log'] .= sprintf(
						__('RapidShare API error: %s', 'psource-link-checker'),
						$response['body']
					);
				}			
