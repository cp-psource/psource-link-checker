...
class blcYouTubeChecker extends blcChecker {
	...
	protected function check_video($response, $result) {
		...
		if ( isset($api['error']) && ($result['http_code'] != 404) ) { //404's are handled later.
			...
			if ( isset($api['error']['message']) ) {
				...
				$result['status_text'] = __('Unknown Error', 'psource-link-checker');
			}
			$result['log'] .= $this->format_api_error($response, $api);

		} else if ( $videoFound ) {
			$result['log'] .= __("Video OK", 'psource-link-checker');
			$result['status_text'] = _x('OK', 'link status', 'psource-link-checker');
			...
			$result['log'] .= __('Video Not Found', 'psource-link-checker');
			$result['broken'] = true;
			$result['http_code'] = 0;
			$result['status_text'] = __('Video Not Found', 'psource-link-checker');
	...
	protected function check_playlist($response, $result) {
		...
		if ( $result['http_code'] == 404 ) {
			//Not found.
			$result['log'] .= __('Playlist Not Found', 'psource-link-checker');
			$result['broken'] = true;
			$result['http_code'] = 0;
			$result['status_text'] = __('Playlist Not Found', 'psource-link-checker');
			$result['status_code'] = BLC_LINK_STATUS_ERROR;
			...
			$result['broken'] = true;
			$result['status_text'] = __('Playlist Restricted', 'psource-link-checker');
			$result['status_code'] = BLC_LINK_STATUS_ERROR;
			...
			if ( empty($api['items']) ) {
				...
				$result['log'] .= __("This playlist has no entries or all entries have been deleted.", 'psource-link-checker');
				$result['status_text'] = _x('Empty Playlist', 'link status', 'psource-link-checker');
				$result['status_code'] = BLC_LINK_STATUS_WARNING;
				$result['http_code'] = 0;
				$result['broken'] = true;
			} else {
				...
				foreach($api['items'] as $video) {
					...
					if ( $is_private ) {
						$result['log'] .= sprintf(
							__('Video status : %s%s', 'psource-link-checker'),
							$video['status']['privacyStatus'],
							''
						);

						$result['broken'] = true;
						$result['status_text'] = __('Video Restricted', 'psource-link-checker');
						$result['status_code'] = BLC_LINK_STATUS_WARNING;
						$result['http_code'] = 0;
						break;
					}
				...
				if ( !$result['broken'] ) {
					//All is well.
					$result['log'] .= __("Playlist OK", 'psource-link-checker');
					$result['status_text'] = _x('OK', 'link status', 'psource-link-checker');
					$result['status_code'] = BLC_LINK_STATUS_OK;
					$result['http_code'] = 0;
				}
